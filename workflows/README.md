...
2. aggregates them accordingly (different operations can be applied to the various variables),
3. regrids the variables to a common grid,
4. executes a pre-trained ML model on such data,
5. masks the results, and
6. evaluates the statistics from the ensemble of results on multiple CMIP6 models.
<p align="justify">The workflow can be started by the following Python command.</p>

```
$ fires(time_range = "2030-01-01_2031-01-01")
```
<p align="justify">CWL and JSON implemetation are also provided. Provenance information can be also produced by PyOphidia, once the execution is completed.</p>
